...
# the wiki id
w3_wiki_id = 'W07ec99131bdd_49f7_8326_7f710cdd4e6a'
...
try:
    ...
    logger.info("Expecting {} total pages in Wiki named '{}', getting {} pages at a time".format(number_of_expected_pages, wiki_title, w3_number_of_pages))
except Exception as e:
...
class NavigationWorker:
    ...
    def get_parent_id(self, the_cookies, the_headers):
        na = requests.get('{}/wikis/basic/api/wiki/{}/navigation/{}/entry?format=json&includeBreadcrumbs=true'.format(w3_host, self.navid, self.pageid), cookies=the_cookies, headers=the_headers)
        pageParentId = json.loads(na.text)['parent']
        return pageParentId


# build out the dictionary with the info of the page
def create_page_append(url, title, link, author, created, modifier, modified, media_link, page_id, attachments, parent_id):
...
if sync_to_confluence:
    ...
    try:
        if use_existing_conf_home_page:
            ...
            homepage_id = create_conf_page('W3 Backup of {}'.format(wiki_title), 'This is the <b>parent</b> page of the wiki backup. Feel free to delete/modify as you see fit!')
...
for i, x in enumerate(pages_to_download):
    if i % 20 == 0 and i != 0:
        logger.info("Searched {} pages for attachments so far, {} pages with attachments".format(i, len(attachments_to_download)))
...
logger.info("Searched {} pages for attachments, {} pages with attachments".format(number_of_pages_to_download, len(attachments_to_download)))
...
if sync_to_confluence:
    ...
    for i, x in enumerate(pages_to_download):
        if i % 20 == 0 and i != 0:
            logger.info("Searched {} pages for comments so far, {} pages with comments".format(i, len(wiki_comment_data)))
...
for i, page in enumerate(pages_to_download):
            ...
            for image_src in soup.find_all('img'):
                ...
                if 'w3-connections.ibm.com' in src_of_imageprint:
                    ...
                    if found_attach_id is not None:
                        ...
                        logger.info("-- Image attachment formatted to Confluence markup in HTML: {}".format(found_attach_id))
    ...
    if sync_to_confluence and replace_connections_files:
        ...
        for link_src in soup.find_all('a', attrs={"_ic_files_uuid": True}):
            ...
            a_link_text = link_src.text
            try:
                conn_link = requests.get("{}/files/basic/anonymous/api/document/{}/entry".format(w3_host, href_of_link), cookies=cookies, headers=headers)
                ...
                if check_conn_file_against_attachments is not None:
                    ...
                    alert_items.append("Had to skip downloading connections file {} on page {} as an attachment already exists for it".format(conn_file_title, page['title']))
                    break
                ...
                if int(conn_file_size) < conf_max_attachment_size:
                    ...
                    with open(os.path.join(local_wiki_directory, page['title'], 'connections_files', conn_file_title), 'wb') as f:
                        f.write(cf.content)
                    create_conf_attachment(conf_page_id, conn_file_title,
                                           os.path.join(local_wiki_directory, page['title'], 'connections_files', conn_file_title))
                    os.chdir(local_wiki_directory)
                    ...
                    try:
                        ...
                        logger.info("Had issue cleaning up superfluous W3 Connections link HTML. See log for more details")
                        logger.error(e, exc_info=logger.getEffectiveLevel() == logging.DEBUG)
                    link_src.replace_with(wiki_link_soup_to_append)
                    logger.info("Connections file link formatted to Confluence markup in HTML: {}".format(conn_file_title))
                else:
                    logger.info("    Unable to download a file of that size as the Confluence limit is {} bytes, skipping".format(conf_max_attachment_size))
                    alert_items.append(
...
if sync_to_confluence:
    ...
    s.auth = (conf_username, conf_password)
        ...
        for p in pages_to_download:
            ...
            try:
                for atta in attachments_to_download[p['page_id']]:
                    attachment_meta_data = attachment_meta_data + "<strong>" + atta['title'] + "</strong>" + " || " + atta['size_human'] +"<br/>"
            except:
                pass
            ...
            pages_rows = pages_rows + """
                    ...
                    """.format(p['title'], p['page_id'], p['author'], p['created'], p['modifier'], p['modified'], p['parent_id'], conf_page_id_str,  p['link'], attachment_meta_data)
